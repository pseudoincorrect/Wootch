...
/**
 * @brief Initiate the Non Volatile Storage
 * 
 * @return true NVS initiated
 * @return false NVS non initiated
 */
...
/**
 * @brief Validate the input for wifi credentials
 * 
 * @param creds wifi credentials struct
 * @return true valid
...
bool app_nvs_validate_wifi_creds(wifi_creds_t *creds)
{
    ...
    if (creds->ssid[MAX_SIZE_CREDS - 1] != 0 || creds->ssid[MAX_SIZE_CREDS - 1] != 0)
    {
        return false;
    }
    return true;
...
/**
 * @brief Get the Wifi credentials from NVS storage
 * 
 * @param creds wifi (output) 
...
/**
 * @brief Write wifi credentials into NVS storage
 * 
 * @param creds Wifi credential (input)
 * @return esp_err_t error code
 */
...
/**
 * @brief Simple test for the NVS storage (useful outside of unit tests)
 */
void app_nvs_test(void)
{
    ...
    if (err != ESP_OK)
        ESP_LOGI(TAG, "app_nvs_get_wifi_creds error %x", err);
    ...
    if (err != ESP_OK)
        ESP_LOGI(TAG, "app_nvs_set_wifi_creds error %x", err);
